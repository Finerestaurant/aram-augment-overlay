...
| 必要なもの | |
|---|---|
| OS | Windows 10 / 11、全画面ならどの解像度でも可、拡大率 100%（実対戦での検証は 1920×1080） |
| OBS Studio | 28 以上 |
...
| ゲーム解像度 | OBS がゲーム画面のサイズを報告できないときに文字を読み取るサイズ。カード位置はどの解像度でも画面に合わせて自動で追従します |
...
- 座標は 1920×1080 で測定しました。クライアントはオーグメント画面を高さ基準で描き、横方向は中央に置く
  ため、ツールも同じ規則で座標を合わせます。1 台のモニターが提供する全画面解像度 15 種（16:9・16:10・
  5:4・4:3、1024×768〜1680×1050）で同じ選択画面をキャプチャして全て確認し、1440p・4K はリサンプルで
  確認しました。実際の対戦は 1920×1080 でしか試しておらず、21:9 以上のウルトラワイドはまだキャプチャが
  ありません。
