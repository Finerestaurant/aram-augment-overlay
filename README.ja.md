...
| 必要なもの | |
|---|---|
| OS | Windows 10 / 11、**16:9 の解像度、拡大率 100%**（実対戦での検証は 1920×1080） |
| OBS Studio | 28 以上 |
...
| ゲーム解像度 | 文字を読み取る画面サイズ。カード位置は画面サイズに自動で追従するため 16:9 ならどのサイズでも動作し、それ以外は警告が出ます |
...
- 座標は 1920×1080 で測定し、受け取った画面サイズに **比率で追従** します。16:9 ならどのサイズでも同じ
  配置になり、セルフテストが録画フレームを 720p・900p・1440p・4K に変えてこれを確認しています。実際の
  対戦は 1920×1080 でしか試していません。16:9 でない画面は OBS が 16:9 に引き伸ばして送るため、その
  場合にカードが合うかは未検証です。
