...
| 需要什么 | |
|---|---|
| 操作系统 | Windows 10 / 11，任意全屏分辨率，缩放 100%（真实对局验证过的是 1920×1080） |
| OBS Studio | 28 或更新 |
| 游戏设置 | 建议**无边框全屏** |
...
| 游戏分辨率 | OBS 无法报告游戏画面尺寸时用于读取文字的尺寸。卡片位置在任何分辨率下都会自行跟随画面 |
...
- 坐标是在 1920×1080 下测得的。客户端按高度绘制强化符文界面并水平居中，工具也按同一规则匹配坐标，
  因此任何分辨率都能工作：在一台显示器提供的全部 15 种全屏分辨率（16:9、16:10、5:4、4:3，从 1024×768
  到 1680×1050）下捕获同一个选择界面并全部验证通过，1440p 和 4K 通过重采样验证。真实对局只在
  1920×1080 下跑过，21:9 及更宽的屏幕尚无捕获。
