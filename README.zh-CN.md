...
> **程序本身的界面只有韩语。** 翻译的只有这份 README，检测阈值也是在韩语客户端上测得的。
> 参见[局限](#局限)。
...
**完全退出 OBS**，运行本程序，在 설정（设置）标签页点击 **OBS websocket 서버 켜기**。在 OBS 里
通过**工具 → WebSocket 服务器设置 → 启用 WebSocket 服务器**操作是等效的。

**2. 运行**

重新启动 OBS，在 상태（状态）标签页点击 **다시 시도**（重试）。连接成功后指示灯变绿，并显示挂件
地址。

窗口的 ✕ 不是退出，而是**收进托盘**。双击托盘图标，或再次启动程序，窗口就会回来。真正要关闭请用
**종료**（退出）按钮，或右键托盘图标选择 종료。
...
改动会写入 exe 旁边的 `config.json`，点击 **저장하고 다시 시작**（保存并重启）后生效。语言和坐标
在启动时只读取一次，所以需要重启。

| | |
|---|---|
| 증강 이름（强化符文名称） | 从 CommunityDragon 获取哪种客户端语言。按语言分别缓存 |
| OCR 언어（OCR 语言） | 用哪种 Windows OCR 语言读屏。**자동**（自动）会跟随上面的语言；缺少语言包时会给出安装命令 |
| 게임 해상도（游戏分辨率） | 坐标按此分辨率换算。16:9 可直接工作，其他比例会给出警告 |
| OBS | websocket 端口 · 游戏采集源名称 · 密码（留空则从 OBS 配置自动读取） |
| 오버레이（覆盖层） | 挂件端口 · 显示行数 · 最大宽度 |
...
**“OBS 연결 안 됨”（未连接到 OBS）**
确认 OBS 正在运行，且 websocket 服务器已打开（설정 标签页的按钮）。启动 OBS 后按
**다시 시도**（重试）即可，不必关掉窗口重开。

**托盘里图标越积越多**
用任务管理器强制结束会让程序没有机会收回图标，于是留下死图标。鼠标从托盘上划过，Windows 就会清理
掉。请用 **종료** 按钮或 `--stop`，不要强制结束。
...
- **全部是在韩语客户端上测量的。** 设置里可以选择其他客户端语言，但读取效果未经验证，程序界面也
  只有韩语。
