...
| 필요한 것 | |
|---|---|
| OS | Windows 10 / 11 · 1920×1080 · 배율 100%. 다른 해상도는 변환식은 있으나 미검증 |
| OBS Studio | 28 이상 |
...
| 상태 | 설정 |
|---|---|
| ![상태 탭](docs/images/app-status.ko.png) | ![설정 탭](docs/images/app-settings.ko.png) |
...
| 설정 | 하는 일 |
|---|---|
| 게임 언어 | 리그 클라이언트의 언어. 증강 이름을 이 언어로 받아오고 화면도 이 언어로 읽습니다. 해당 OCR 언어 팩이 없으면 바로 설치할 수 있습니다. 처음에는 리그 설치본에서 읽어오고, 못 읽을 때만 윈도우 표시 언어를 씁니다 |
| 게임 해상도 | OBS 가 게임 화면 크기를 알려주지 못할 때 글자를 읽을 크기. 검증된 값은 1920×1080 뿐이고, 카드 위치는 어느 크기에서도 화면에 맞춰 따라갑니다 |
...
- 디스플레이 배율이 100%인지 확인
- 게임이 **테두리 없는 전체화면** 또는 전체화면인지 확인
- 상태 줄에 **OBS에 게임 화면이 없습니다** 라고 뜨면 게임 캡처 소스가 게임 창을 잡지 못한 것입니다.
  도구가 한 번 스스로 바로잡고, 그래도 안 되면 소스의 창 목록에서 “League of Legends (TM) Client” 를
  고르세요

**엉뚱한 증강이 올라갔습니다**

설정 › 고급에서 **판정 기록 남기기** 를 켜고 게임을 하세요. 그러면 증강 선택창이 열린 순간부터 닫힐
때까지가 전부 남고, `http://127.0.0.1:8777/inspect` 에서 한 프레임씩 되돌려 볼 수 있습니다. 화면 위에
감지가 실제로 읽는 모든 박스가 그려지고, 그 프레임의 판정값이 기준값과 나란히 표시됩니다 — 리롤 버튼
게이트 점수, 카드 밝기, 선택 섬광의 두 가지 검사, 툴팁 패널, 그리고 리롤과 판정 프레임이 찍힌
타임라인입니다. 좌우 화살표가 한 프레임, Shift 를 누르면 열 프레임입니다.

창 하나가 15~25MB 라서 기본은 꺼짐입니다. `state/inspect` 에 최근 8개를 보관하고 오래된 것부터
지웁니다.
...
2. OBS websocket으로 게임 화면을 받아 **새로고침 버튼 3개를 템플릿 매칭** → 증강 선택창 감지
...
- **검증된 해상도는 1920×1080 뿐입니다.** 좌표를 측정한 해상도이고, 실제 게임을 돌려본 유일한
  해상도이며, 설정에서 제공하는 유일한 값입니다. 변환식 자체는 어떤 크기에서도 동작하도록 쓰였고
  계산은 검증돼 있습니다 — 클라이언트가 증강 화면을 높이 기준으로 그리고 가로 중앙에 놓으므로,
  한 모니터가 제공하는 전체화면 15개 해상도(16:9·16:10·5:4·4:3, 1024×768~1680×1050)에서 같은
  증강 창을 캡처해 전부 바르게 읽었고, 1440p·4K 리샘플까지 더해 두 가지 모두 자동 검사로
  묶여 있습니다. 다만 **그 해상도들로 게임을 해 본 것은 아닙니다.** 21:9 이상 울트라와이드는
  캡처조차 없고, HUD 스케일도 시험해 보지 않았습니다.
...
- **임계값은 한국어와 영어 클라이언트에서 측정했고, 둘 다 실제로 플레이하며 확인했습니다.**
  일본어와 중국어도 설정에서 고를 수 있고 같은 측정값을 쓰지만, 여러 판에 걸친 검증은
  하지 않았습니다.
