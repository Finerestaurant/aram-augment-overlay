...
| 필요한 것 | |
|---|---|
| OS | Windows 10 / 11, **16:9 해상도, 배율 100%** (실제 게임 검증은 1920×1080) |
| OBS Studio | 28 이상 |
...
| 게임 해상도 | 글자를 읽는 화면 크기. 카드 위치는 화면 크기에 맞춰 스스로 따라가므로 16:9 면 어느 크기든 동작하고, 아니면 경고가 뜹니다 |
...
- 좌표는 1920×1080 기준으로 측정했고, 받은 화면 크기에 맞춰 **비율로 따라갑니다**. 16:9 라면 어느
  크기든 같은 배치로 떨어지고, 셀프테스트가 녹화 프레임을 720p·900p·1440p·4K 로 바꿔 이를 확인합니다.
  실제 게임은 1920×1080 에서만 돌려봤습니다. 16:9 가 아닌 화면은 OBS 가 16:9 로 늘려서 보내므로, 그때도
  카드가 맞는지는 검증되지 않았습니다.
