...
| 필요한 것 | |
|---|---|
| OS | Windows 10 / 11, 전체화면 해상도 무관, 배율 100% (실제 게임 검증은 1920×1080) |
| OBS Studio | 28 이상 |
...
| 게임 해상도 | OBS 가 게임 화면 크기를 알려주지 못할 때 글자를 읽을 크기. 카드 위치는 어느 해상도에서도 화면에 맞춰 스스로 따라갑니다 |
...
- 좌표는 1920×1080 기준으로 측정했습니다. 클라이언트가 증강 화면을 높이 기준으로 그리고 가로 중앙에
  놓으므로 도구도 같은 규칙으로 좌표를 맞춥니다. 한 모니터가 제공하는 전체화면 15개 해상도(16:9·16:10·
  5:4·4:3, 1024×768~1680×1050)에서 같은 증강 창을 캡처해 전부 확인했고, 1440p·4K 는 리샘플로 확인했습니다.
  실제 게임은 1920×1080 에서만 돌려봤고, 21:9 이상 울트라와이드는 아직 캡처가 없습니다.
